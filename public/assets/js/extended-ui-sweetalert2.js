...
(function () {
  const basicAlert = document.querySelector('#basic-alert'),
    ...
    confirmColors = document.querySelectorAll('.delete-button');
  ...
  if (confirmText) {
    ...
    confirmColors.forEach(confirmColor => {
        confirmColor.onclick = function (event) {
            event.preventDefault(); // Mencegah tautan default
            const id = confirmColor.getAttribute('data-id');

            // Pastikan ID tidak null atau undefined
            if (!id) {
                console.error('Data ID tidak ditemukan atau null');
                return;
            }
